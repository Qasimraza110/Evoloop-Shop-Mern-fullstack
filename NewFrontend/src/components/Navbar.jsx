  ...
  return (
    <nav className="bg-black text-white px-4 md:px-8 py-4 flex justify-between items-center relative z-50">
      ...
      <div className="hidden md:flex items-center gap-6">
        ...
        {/* Cart with badge */}
        <Link to="/cart" className="relative hover:text-yellow-400 transition-colors duration-200 flex items-center gap-1">
      ...
      {mobileMenu && (
        <div className="absolute top-full left-0 w-full bg-black flex flex-col items-center gap-4 py-4 md:hidden">
          ...
          <Link to="/cart" onClick={() => setMobileMenu(false)} className="relative hover:text-yellow-400 transition-colors duration-200 flex items-center gap-1">
            <FaShoppingCart className="text-yellow-400" /> Cart
            {cartCount > 0 && (
              <span className="absolute -top-2 -right-3 bg-red-600 text-white text-xs font-bold rounded-full px-2 py-0.5">
                {cartCount}
