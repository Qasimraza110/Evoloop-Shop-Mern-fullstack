...
export default function ProductsMain() {
  ...
  const fetchProducts = async (pageNumber = 1, searchQuery = "", categoryFilter = "all") => {
    setLoading(true);
    try {
      ...
      if (priceFilter === "50to100")
        fetchedProducts = fetchedProducts.filter((p) => p.price >= 50 && p.price <= 100);
      ...
      const q = {};
      fetchedProducts.forEach((p) => (q[p._id] = 1));
      setQuantities(q);
    } catch (err) {
  ...
  const updateQuantity = (id, change, stock) => {
    setQuantities((prev) => {
      const newQty = Math.min(Math.max((prev[id] || 1) + change, 1), stock || 9999);
                    ...
                    >
                      ...
                      <div className="p-4 flex flex-col flex-grow">
                        ...
                        <div className="mb-2">
                          ...
                          >
                            <Link to={`/product/${p._id}`} className="hover:text-blue-800  text-black">
                              {p.name}
                            </Link>
                          </h3>
                          ...
                          {p.name.length > 30 && (
                            <button
                              className="text-blue-600 text-xs font-medium mt-1 transition hover:text-blue-800 "
                              onClick={() =>
